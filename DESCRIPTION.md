...
- Add `normalize_referred_schema=true` dialect parameter (URL or dialect keyword argument): opt-in that makes foreign key reflection return `referred_schema=None` for FK targets in the connection's default/current schema, matching SQLAlchemy's reflection convention so Alembic autogenerate does not produce spurious migrations for unqualified default-schema references. Targets in non-default schemas always keep their actual schema so SQLAlchemy's `_reflect_fk` autoloads them from the correct place. The previous behavior (returning the schema name for every FK target) is preserved by default for backward compatibility ([#610](https://github.com/snowflakedb/snowflake-sqlalchemy/issues/610), SNOW-2313675).
