...
- Add support for row access policies on `SnowflakeTable` via the `row_access_policy` parameter, allowing users to specify `SnowflakeTable(..., row_access_policy=('policy_name', ['col1', 'col2']))` to render `WITH ROW ACCESS POLICY policy_name ON (col1, col2)` in CREATE TABLE (SNOW-884382 / GH #432).
