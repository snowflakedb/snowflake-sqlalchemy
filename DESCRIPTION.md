...
- Improve `_url()` helper and `create_connect_args` connection-parameter handling:
  ...
  - A curated set of connector kwargs (`host`, `protocol`, `token_file_path`, `private_key_file`, `ocsp_response_cache_filename`, `connection_diag_log_path`, `crl_cache_dir`, `unsafe_file_write`, `unsafe_skip_file_permissions_check`) is no longer accepted when supplied via the URL query string, directing callers to the `connect_args=` keyword argument of `create_engine()` instead — the path that is fully under application control. Applications already using `connect_args=` for these parameters are unaffected.
  - Applications that cannot immediately migrate may re-enable the previous behaviour with the `legacy_url_params=True` engine kwarg (`create_engine(url, legacy_url_params=True)`) or, as a global fallback, the `SNOWFLAKE_SQLALCHEMY_LEGACY_URL_PARAMS=true` environment variable. An explicit kwarg takes precedence over the environment variable. While enabled, the affected parameters are forwarded as before and a `DeprecationWarning` is emitted for each one. The flag is intentionally not honoured as a URL query parameter, and the shim will be removed in the next major release — after which the blocked parameters always raise `ArgumentError` when supplied via the URL query string, with no opt-out.
