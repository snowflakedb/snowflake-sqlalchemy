...
- Add opt-in `enable_structured_type_json` dialect flag (URL parameter or `create_engine(..., enable_structured_type_json=True)`). When enabled, reading `VARIANT`, `OBJECT`, `ARRAY` and `MAP` columns deserializes the JSON text Snowflake returns into native Python (`dict`/`list`/...), using the engine's `json_deserializer` when provided. Off by default, so existing code that reads raw JSON strings is unchanged (SNOW-1820647 / GH #107).
...
- Add support for row access policies on `SnowflakeTable` via the `row_access_policy` parameter; the policy name accepts a string (optionally fully qualified, e.g. `'db.schema.policy'`) or a `snowflake.sqlalchemy.FQN`. See [Row Access Policy Support](README.md#row-access-policy-support) (SNOW-884382 / GH #432).
...
- v2.0.0a0 (July 16, 2026)
  ...
  - Add a [contributing guide](https://github.com/snowflakedb/snowflake-sqlalchemy/blob/main/CONTRIBUTING.md) for local development setup, Hatch usage, and running tests.
